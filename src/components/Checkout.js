...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let newAddressData = await axios.post(`${config.endpoint}/user/addresses`,{"address":newAddress?.value},{headers: {
        Authorization: `Bearer ${token}`,
      }})
      setAddresses({...addresses,all: newAddressData?.data})
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      let deleteAddressData = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,{headers: {
        Authorization: `Bearer ${token}`,
      }})
      setAddresses({all: deleteAddressData?.data,selected:""})
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      if(error.response){
        ...
        enqueueSnackbar("Could not placed order. Check that balance is running",{variant:"error"})
      }
    } 
  };
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const getAdress = await getAddresses(token);
    };
    onLoadHandler();
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */
                addresses?.all ? 
                  addresses?.all?.map((address)=>(
                    <Box className={ address?._id === addresses.selected ? "address-item selected" : "address-item nor-selected"} key={address?._id} onClick={()=>{
                      setAddresses({...addresses,selected:address?._id})
                    }}>
                      <Typography>{address?.address}</Typography>
                      <Button startIcon={<Delete />} sx={{color: "green"}} onClick={async()=>{
            ...
            {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Dislay either "Add new address" button or the <AddNewAddressView> component to edit the currently selected address */}
            {
              ...
              size="large"
              onClick={()=>{
                setNewAddress((curentNewAddress)=>({
                  ...curentNewAddress,
            ...
            <Box my="1rem">
              ...
              <Typography>
                Pay ${parseInt(getTotalCartValue(items))} of available $
